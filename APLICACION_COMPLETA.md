...
## Diseño visual, Poder, figuras mundiales y oleada 4 (2026-09-25)
- **Interfaz**: kit reutilizable en `src/components/ui/` (`Modal` = hoja inferior en móvil / tarjeta en escritorio con Escape y bloqueo de scroll; `StatBar` con estela de daño; `ChatFeed` + `TypingIndicator`; `Toasts` con los cambios de nivel, rango, berries, vida y recompensa calculados por `lib/ui/format.ts` (`diffVitals`, con tests); `WantedPoster`; `BackToCharacter`; `SeaBackground`). La pantalla de juego se dividió en componentes (`PlayHeader` fijo con vida y aguante en móvil, `ScenePanel` con la escena y la caja de texto juntas, `DuelPanel`, `JointFightPanel`, `WorldPanels`, `PeoplePanels`, `IslandCard`, `CharacterSheet`). Tailwind v4: `border-[--line]` no funcionaba (bordes blancos); ahora hay tokens `line`, `jade`, `fruit`, `stamina`, `parchment`. Animaciones con `prefers-reduced-motion`. Portada, lista de personajes y creación rediseñadas. `devIndicators: false` (el botón "N" de desarrollo tapaba las hojas en móvil).
- **Poder** (`engine/sovereignty.ts` + tests, `game/sovereignty.ts`, `api/characters/[id]/sovereignty`, `SovereigntyPanel.tsx`): Yonko (requisitos, proclamación con trono vacío o desafío en persona a un Yonko canon donde esté; pelea en grupo de tipo `sovereign`; al ganar, el canon pierde el trono (`role` NOTABLE_PIRATE, `rankLabel` "Ex-Yonko") y sus islas pasan al jugador; muerte o captura = `WorldArc` en AWAITING_CONSENT para `/admin`; reacción del mundo: rencores de Yonko y almirantes y Buster Call), Shichibukai (7 asientos contando los canon activos sin "Ex-", tributo semanal del 1%, recompensa congelada, la Marina y el CP-0 no pueden cazarles ni arrestarles, revocación por impago, por matar a un agente del Gobierno o por renuncia) y guerras (`War`: contra la Marina en sus bases, con el almirante presente si está; contra otro Yonko jugador tomando sus islas; 3 golpes o 7 días).
- **Figuras mundiales** (`isWorldFigure`, `reportFigure`, `Character.lastFigureNewsAt`): sus desembarcos salen en "Figuras del mundo", con límite de 45 min. La Marina llega a "Almirante de Flota". El narrador sabe si el personaje es Yonko o Shichibukai.
