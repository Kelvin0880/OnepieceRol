...
**Canon encounters, prison arcs, party rounds, zero invented characters (2026-09-26, later)**:
- Panel "Gente de <isla>" on the play screen (`play/[id]/IslandPeople.tsx`, data `canonHere`/`islandCast` in the character GET): canon characters in plain sight on your island (ask for a task = `Mission.patronActorId`, or challenge them) and the island's residents with live state. Engine `engine/canon-encounter.ts`, game `game/canon-encounter.ts`, route `api/characters/[id]/canon` (ops mission|challenge|duel|verdict), table `CanonChallenge`.
- Challenge = joint fight kind `canon_vanguard` against a REAL subordinate (same crew/faction below them, `pickVanguard`; else the strongest free resident; else straight to the duel), then kind `canon` against the character in person (`isActor`, no auto humiliation). Winner picks capture/kill/spare: spare is immediate; capture/kill create a `WorldArc` kind `player_verdict` (AWAITING_CONSENT, `requestedChoice`) that ONLY the owner decides in /admin (3 buttons, can override); `onPlayerVerdictDecided` pays and tells the player. Yonko are challenged through the throne system, Gorosei/hidden ruler not at all, civilians cannot be dueled, you never go against your own side, one challenger per canon at a time.
- Prison arcs: captured canon characters try to get out by themselves (`breakout`) or with their crew's strongest free member (`crew_rescue`), 5 chapters set in Impel Down, decided at the end by `judgeMatch` (`resolvePrisonArc`), no owner verdict: freed = ACTIVE + hidden, held = stays. Doflamingo starts in cell 6 (`CANON_PRISONERS`, `placeCanonPrisoners`, applied by the seed and by `scripts/seed-residents.ts` without touching any other canon status).
- Zero invented characters: the narrator, referee (solo, joint fights, duels), party scene, news, world events, digests, briefings, Coliseum, duel reports, world happenings and events all reject text that presents a named character outside residents/canon/players/places (`inventedNames`, `allowedNamesAt/Everywhere`); recruiting only accepts a free island resident (never a made-up or canon name; the resident leaves and the tick sends a successor); beginner-event rivals are real residents; names already established in the running scene stay allowed.
- Shared crew scene is now ROUNDS, not turns (`engine/party-round.ts`, `game/party-round.ts`, `Party.roundActionsJson`): the narrator opens, every member writes (their line is in the feed at once), the narrator answers ALL actions in one message; anyone can close the round (`party_close_round`), a round idle for 15 min is answered anyway. Personal actions (explore, attack...) are never blocked by the round.
- Checks: `canon-check.ts`, `prison-arc-check.ts`, `party-round-check.ts` (DB), `canon-ui-check.mjs` (390 px), `party-multiplayer-smoke.mjs` (rewritten for rounds).
