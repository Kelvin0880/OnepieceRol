...
async function settleJointFight(fightId: string, outcome: "victory" | "defeat" | null, enemy: JointEnemy, rewards: JointRewards) {
  ...
  if (fight.kind === "rescue") {
    const { handleRescueSettled } = await import("./rescue-raid");
    const fresh = await prisma.jointFight.findUniqueOrThrow({ where: { id: fightId }, include: { participants: true } });
    closing.push(...(await handleRescueSettled({ contextJson: fresh.contextJson, outcome, humans: fresh.participants.filter((p) => !p.isNpc).map((p) => ({ characterId: p.characterId, status: p.status, name: p.name })) })));
  }
