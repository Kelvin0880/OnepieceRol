...
export interface EventInput {
  islands: { name: string; danger: number; residents?: string[] }[];
  recentTitles: string[];
  maxLevel: number;
...
const EVENT_SYSTEM =
  ...
  "Reglas: nada de muertes ni capturas de personajes canon; el premio lo fija el sistema (no lo inventes); sitúalo en UNA isla de la lista, tal cual se llama; NO inventes personajes con nombre: menciona solo a los habitantes de la isla que se te dan o usa gente anónima. " +
  "Los rivales de la prueba los pone el sistema (habitantes reales de la isla): déjalos como lista vacía. " +
  'Responde SOLO JSON: {"title":"título corto","island":"nombre exacto","description":"anuncio de 80 a 160 palabras que explique la prueba y qué debe escribir el participante","rivals":[]}.';
...
export async function inventEvent(input: EventInput): Promise<EventDraft> {
  ...
  const user =
    `Islas del mundo:\n${input.islands.map((i) => `- ${i.name} (peligro ${i.danger})${i.residents?.length ? ` — habitantes: ${i.residents.join("; ")}` : ""}`).join("\n")}\n\n` +
  ...
  try {
    const raw = await callOpenRouter(EVENT_SYSTEM, user, { models: OPENROUTER_MODELS, jsonMode: true, timeoutMs: 75_000, maxTokens: 1200, validate: (t) => { const d = parseEventDraft(t, names, input.maxLevel); return d !== null && inventedNames(`${d.title} ${d.description}`, allowed).length === 0; } });
