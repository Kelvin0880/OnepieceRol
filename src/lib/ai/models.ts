 ...
 * never used. DeepSeek V3.2 answers first (long, structured, creative attacks per cent); the cheap gpt-4o-mini joins as a
 * hedged backup when it is slow or fails (see hedgeDelayMs). This list is the only place that knows model names.
 */
const DEFAULT_MODELS = ["deepseek/deepseek-v3.2", "openai/gpt-4o-mini"];
